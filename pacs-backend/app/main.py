...
# Configure multipart parser to handle more fields
from multipart import MultipartParser

MultipartParser.max_fields = 10000  # Increase field limit to 10,000
...
if "*" in allowed_origins:
    ...
    warnings.warn(
        "CORS allows all origins - this is insecure for production!", UserWarning
    )
...
def is_rate_limited(ip_address: str) -> bool:
    ...
    login_attempts[ip_address] = [
        attempt_time
        for attempt_time in login_attempts[ip_address]
...
class LargeUploadMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        if request.url.path.startswith(
            "/studies/upload"
        ) or request.url.path.startswith("/api/studies/upload"):
