...
REQUEST_COUNT = Counter(
    "pacs_requests_total", "Total requests", ["method", "endpoint", "status"]
)
...
DICOM_PROCESSING_TIME = Histogram(
    "pacs_dicom_processing_seconds", "DICOM processing time"
)
    ...
    async def wrapper(*args, **kwargs):
        ...
        try:
            ...
            REQUEST_COUNT.labels(
                method="POST", endpoint=func.__name__, status="success"
            ).inc()
            return result
        except Exception as e:
            REQUEST_COUNT.labels(
                method="POST", endpoint=func.__name__, status="error"
            ).inc()
            raise
        finally:
