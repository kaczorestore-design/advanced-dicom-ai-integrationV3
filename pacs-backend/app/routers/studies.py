...
from ..auth import (
    get_current_user, 
    require_admin, 
    require_diagnostic_center_admin,
...
from ..utils import generate_study_id
...
async def upload_study(
    ...
    if current_user.role not in [UserRole.TECHNICIAN, UserRole.DOCTOR]:
        raise HTTPException(
            status_code=403, detail="Only technicians and doctors can upload studies"
        )
    ...
    if not gender and extracted_metadata.get("patient_sex"):
        gender = (
            "M"
            if extracted_metadata["patient_sex"].upper() == "M"
            else "F"
            if extracted_metadata["patient_sex"].upper() == "F"
            else "O"
        )
        ...
        if file.filename and file.filename.lower().endswith(".dcm"):
            ...
            try:
                ...
                if "MediaStorageSOPClassUID" not in ds.file_meta:
                    ds.file_meta.MediaStorageSOPClassUID = ds.get(
                        "SOPClassUID", "1.2.840.10008.5.1.4.1.1.2"
                    )
                if "MediaStorageSOPInstanceUID" not in ds.file_meta:
                    ds.file_meta.MediaStorageSOPInstanceUID = ds.get(
                ...
                dicom_file = DicomFile(
                    ...
                    slice_number=int(ds.get("InstanceNumber", 0))
...
async def get_pending_studies(
    ...
    if current_user.role not in [UserRole.RADIOLOGIST, UserRole.ADMIN]:
        raise HTTPException(
            status_code=403,
            detail="Only radiologists and admins can view pending studies",
        )

    studies = (
        db.query(Study)
        .filter(Study.status.in_([StudyStatus.QUEUED, StudyStatus.PROCESSING]))
        .all()
    )
    ...
    for study in studies:
        ...
        center = (
            ...
            .filter(DiagnosticCenter.id == study.diagnostic_center_id)
            .first()
        )
...
async def get_study(
    ...
    # Universal medical access - all authenticated medical professionals can view studies
    # Only check that user has a valid medical role
    ...
    print(
        f"Medical access granted - User {current_user.id} ({current_user.role.value}) "
        f"from center {current_user.diagnostic_center_id} accessed study {study_id} "
...
async def get_dicom_file(
    ...
    # Universal medical access - all authenticated medical professionals can view DICOM files
    if current_user.role not in [
        UserRole.ADMIN,
        UserRole.RADIOLOGIST,
        UserRole.DOCTOR,
    ...
    # Log access for audit purposes
    print(
        f"DICOM access granted - User {current_user.id} ({current_user.role.value}) "
        f"from center {current_user.diagnostic_center_id} accessed DICOM file {file_id} "
        f"from study {study.id} (center {study.diagnostic_center_id})"
    )
...
async def create_deletion_request(
    ...
    if current_user.role not in [UserRole.TECHNICIAN, UserRole.DOCTOR]:
        raise HTTPException(
            status_code=403, detail="Only technicians and doctors can request deletions"
        )
...
async def get_deletion_requests(
    ...
    if current_user.role != UserRole.ADMIN:
        raise HTTPException(
            status_code=403, detail="Only admins can view deletion requests"
        )
...
async def approve_deletion_request(
    ...
    if current_user.role != UserRole.ADMIN:
        raise HTTPException(
            status_code=403, detail="Only admins can approve deletion requests"
        )
...
async def reject_deletion_request(
    ...
    if current_user.role != UserRole.ADMIN:
        raise HTTPException(
            status_code=403, detail="Only admins can reject deletion requests"
        )
...
async def assign_study_to_self(
    ...
    if current_user.role != UserRole.RADIOLOGIST:
        raise HTTPException(
            status_code=403, detail="Only radiologists can assign studies to themselves"
        )
    ...
    if study.radiologist_id:
        raise HTTPException(
            status_code=400, detail="Study already assigned to a radiologist"
        )
...
async def get_study_status(
    ...
    # Universal medical access - all authenticated medical professionals can view study status
    if current_user.role not in [
        UserRole.ADMIN,
        UserRole.RADIOLOGIST,
        UserRole.DOCTOR,
    ...
    # Log access for audit purposes
    print(
        f"Study status access granted - User {current_user.id} ({current_user.role.value}) "
        f"from center {current_user.diagnostic_center_id} accessed status for study {study_id} "
        f"from center {study.diagnostic_center_id}"
    )
    ...
    technician = db.query(User).filter(User.id == study.uploaded_by_id).first()
    radiologist = (
        db.query(User).filter(User.id == study.radiologist_id).first()
        if study.radiologist_id
        else None
    )
    ...
    status_info = {
        ...
        "timeline": [
            ...
            },
            {
                ...
                "status": "completed"
                if study.status in [StudyStatus.COMPLETED, StudyStatus.REVIEWED]
                else "pending",
                "timestamp": None,
            },
        ],
...
async def update_report(
    ...
    report_content = {
        ...
        "radiologist_info": {
            ...
            "license_number": current_user.medical_license_number,
            "board_certification": current_user.board_certification,
            "certification_expiry": current_user.certification_expiry.isoformat()
            if current_user.certification_expiry
            else None,
        }
        if current_user.role == UserRole.RADIOLOGIST
        else None,
...
async def download_report_pdf(
    ...
    p.drawString(
        50, 50, f"Report generated on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
...
async def share_report_with_patient(
    ...
    try:
        ...
        msg[
            "Subject"
        ] = f"Medical Report Available - {patient.first_name} {patient.last_name}"
...
async def update_patient(
    ...
    """Update patient information with universal medical access"""
    ...
    log_audit_event(
        ...
        details={
            ...
            "user_center": current_user.diagnostic_center_id,
...
async def get_patient(
    ...
    """Get patient information with universal medical access"""
