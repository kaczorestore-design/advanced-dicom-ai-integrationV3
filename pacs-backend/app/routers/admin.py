...
async def get_system_monitoring(
    ...
    recent_uploads = (
        db.query(DicomFile).filter(DicomFile.created_at >= yesterday).count()
    )
        ...
        "center_storage": [
            {
                ...
                "usage_percent": round(
                    ((center.storage_used_gb or 0) / (center.storage_quota_gb or 100))
                    * 100,
                    2,
                ),
        ...
        "recent_logs": [
            {
                ...
                "level": "info"
                if "success" in log.action.lower()
                else "warning"
                if "warning" in log.action.lower()
                else "error",
            }
...
async def get_center_analytics(
    ...
    studies_by_status = (
        db.query(Study.status, func.count(Study.id).label("count"))
        .filter(Study.diagnostic_center_id == center_id)
        .group_by(Study.status)
        .all()
    )

    studies_by_modality = (
        db.query(Study.modality, func.count(Study.id).label("count"))
        .filter(Study.diagnostic_center_id == center_id)
    ...
    recent_activity = (
        db.query(Study)
        .filter(Study.diagnostic_center_id == center_id, Study.created_at >= last_week)
    ...
    staff_stats = (
        ...
        .join(Study, User.id == Study.uploaded_by_id)
        .filter(User.diagnostic_center_id == center_id)
    ...
    return {
        ...
        "studies_by_status": [
            {"status": status, "count": count} for status, count in studies_by_status
        ],
        "studies_by_modality": [
        ...
        "staff_performance": [
            {"user_id": user_id, "name": name, "role": role, "studies_uploaded": count}
